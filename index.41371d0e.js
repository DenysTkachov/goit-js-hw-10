...
let tw={};function tA(t,e,r){tw[t]=class extends r{constructor(){super(),Object.defineProperty(this,"message",{value:e.apply(this,arguments),writable:!0,configurable:!0}),// Add the error code to the name to include it in the stack trace.
...
// ================
let tS=/[^+/0-9A-Za-z-_]/g;function tT(t,e){let r;e=e||1/0;let n=t.length,i=null,o=[];for(let s=0;s<n;++s){// is surrogate component
 ...
 *
 * @returns {boolean}
 */function tj(t){return q.isPlainObject(t)||q.isArray(t)}/**
 * It removes the brackets from the end of a string
 *
 * @param {string} key - The key of the parameter.
 *
 * @returns {string} the key without the brackets.
 */function t_(t){return q.endsWith(t,"[]")?t.slice(0,-2):t}/**
...
t=t_(t),!r&&e?"["+t+"]":t)}).join(r?".":""):e}let tF=q.toFlatObject(q,{},null,function(t){return/^is[A-Z]/.test(t)});var tD=/**
...
t=JSON.stringify(t);else{var u;if(q.isArray(t)&&(u=t,q.isArray(u)&&!u.some(tj))||(q.isFileList(t)||q.endsWith(r,"[]"))&&(a=q.toArray(t)))return(// eslint-disable-next-line no-param-reassign
r=t_(r),a.forEach(function(t,n){q.isUndefined(t)||null===t||e.append(!0===s?tP([r],n,o):null===s?r:r+"[]",f(t))}),!1)}}return!!tj(t)||(e.append(tP(i,r,o),f(t)),!1)}let c=[],h=Object.assign(tF,{defaultVisitor:l,convertValue:f,isVisitable:tj});if(!q.isObject(t))throw TypeError("data must be an object");return!function t(r,n){if(!q.isUndefined(r)){if(-1!==c.indexOf(r))throw Error("Circular reference detected in "+n.join("."));c.push(r),q.forEach(r,function(r,o){let s=!(q.isUndefined(r)||null===r)&&i.call(e,r,q.isString(o)?o.trim():o,n,h);!0===s&&t(r,n?n.concat(o):[o])}),c.pop()}}(t),e};/**
...
let t0=q.toObjectSet(["age","authorization","content-length","content-type","etag","expires","from","host","if-modified-since","if-unmodified-since","last-modified","location","max-forwards","proxy-authorization","referer","retry-after","user-agent"]);var /**
...
let{Axios:eB,AxiosError:eS,CanceledError:eT,isCancel:eU,CancelToken:eC,VERSION:eL,all:ex,Cancel:eI,isAxiosError:eN,spread:ej,toFormData:e_,AxiosHeaders:eP,HttpStatusCode:eF,formToJSON:eD,getAdapter:ek,mergeConfig:eM}=eR;var e$=f("bOlkj");eR.defaults.headers.common["x-api-key"]="live_gemlNDVGJLAl0mz7HH50hfC9TVvG9ZcMn2NIH4ZNebsWKsZAcTmzySLkJh5Z0a6P";let eq=document.querySelector(".breed-select"),ez=document.querySelector(".loader"),eH=document.querySelector(".error"),eJ=document.querySelector(".cat-info");window.addEventListener("load",function(){let t;(0,e$.fetchBreeds)().then(e=>{t=e,e.forEach(t=>{let e=document.createElement("option");e.value=t.id,e.textContent=t.name,eq.appendChild(e),eq.classList.remove("is-hidden"),ez.classList.add("is-hidden")})}).catch(t=>{console.log(t),eH.classList.remove("is-hidden"),eq.classList.add("is-hidden"),ez.classList.add("is-hidden")}),eq.addEventListener("change",()=>{let e=eq.value;ez.classList.remove("is-hidden"),eJ.classList.add("is-hidden"),eH.classList.add("is-hidden"),(0,e$.fetchCatByBreed)(e).then(r=>{let n=t.find(t=>t.id===e),i=`<img src='${r.url}' width='300' alt='${n.name}'/>
        <div class='textInfo'><h1>${n.name}</h1>
        <p>${n.description}</p>
        <p><b>Temperament:</b> ${n.temperament}</p></div>`;eJ.innerHTML=i,ez.classList.add("is-hidden"),eJ.classList.remove("is-hidden")}).catch(t=>{console.log(t),eH.classList.remove("is-hidden"),ez.classList.add("is-hidden")})})})}();//# sourceMappingURL=index.41371d0e.js.map

//# sourceMappingURL=index.41371d0e.js.map
